...
public:
	...
	Appender operator() (Appender app, const Args&... args)
	{
		...
		parser(_fmt.begin(), _fmt.end(), sizeof...(Args),
			[&] (format_iterator first, format_iterator last) { app.append(addressof(*first), distance(first, last)); },
			[&] (size_t n, size_t arg, format_iterator first, format_iterator last)
			{
				auto flags = format_type{ first, distance(first, last) };
